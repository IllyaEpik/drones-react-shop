...
export function useProduct(defaultCount:number,defaultSkip:number,defaultSort:string): [
        IProduct[] | undefined, 
        boolean, 
        unknown, 
        {
    ...
    useEffect(() => {   
        const fetchData = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:8000/products/?count=${count}&skip=${skip}&sort=${sort}`)
